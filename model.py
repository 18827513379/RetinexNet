...
def DecomNet(input_im, layer_num, channel=64, kernel_size=3)://定义分解网络，参数有输入，卷积层数量，滤波器通个数，卷积核尺寸
    input_max = tf.reduce_max(input_im, axis=3, keepdims=True)//将输入图片减少到3维 255*255*3？
...
//最后一个卷积层，通道数为什么是4，没有用到激活函数，叫重建层？
    R = tf.sigmoid(conv[:,:,:,0:3]) // 分解为R和L分量
    L = tf.sigmoid(conv[:,:,:,3:4])

    return R, L

def RelightNet(input_L, input_R, channel=64, kernel_size=3)://增强网络，通道数64，卷积核尺寸3*3
    input_im = concat([input_R, input_L])//恢复为原来的图像形式，
    with tf.variable_scope('RelightNet'):
        ...
        //提取图像的局部特征
        up1 = tf.image.resize_nearest_neighbor(conv3, (tf.shape(conv2)[1], tf.shape(conv2)[2]))//加入最邻近插值
        deconv1 = tf.layers.conv2d(up1, channel, kernel_size, padding='same', activation=tf.nn.relu) + conv2//跳跃连接
        ...
        feature_gather = concat([deconv1_resize, deconv2_resize, deconv3])//连接到
        feature_fusion = tf.layers.conv2d(feature_gather, channel, 1, padding='same', activation=None) //1*1的卷积核
...
class lowlight_enhance(object):
    def __init__(self, sess):
        self.sess = sess
        self.DecomNet_layer_num = 5//分解网络5层

        # build the model
        //神经网络构建graph的时候在模型中的占位，此时并没有把要输入的数据传入模型，它只会分配必要的内存
        ...
        [R_low, I_low] = DecomNet(self.input_low, layer_num=self.DecomNet_layer_num)//返回分解网分解的分量
        [R_high, I_high] = DecomNet(self.input_high, layer_num=self.DecomNet_layer_num)
        
        I_delta = RelightNet(I_low, R_low)//返回增强后的I分量

        I_low_3 = concat([I_low, I_low, I_low])将这些分量连接为原来的维度？保持维度一致？
        ...
        self.output_S = R_low * I_delta_3//合成增强后的光照度
    ...
    def gradient(self, input_tensor, direction)://创建一个常量
        self.smooth_kernel_x = tf.reshape(tf.constant([[0, 0], [-1, 1]], tf.float32), [2, 2, 1, 1])//重塑形状，格式[2, 2, 1, 1]
    ...
    def evaluate(self, epoch_num, eval_low_data, sample_dir, train_phase):
        ...
        for idx in range(len(eval_low_data)):
            input_low_eval = np.expand_dims(eval_low_data[idx], axis=0)//用于扩展数组的形状,在0位置添加数据,数组形状变为0的位置是1.[1 ? ? ?]
        ...
        for epoch in range(start_epoch, epoch):
            for batch_id in range(start_step, numBatch):
                ...
                for patch_id in range(batch_size):
                    h, w, _ = train_low_data[image_id].shape//图片形状大小
                    x = random.randint(0, h - patch_size)//255-48?
                    y = random.randint(0, w - patch_size)
            
                    rand_mode = random.randint(0, 7)//7种变换模式
                    ...
                    if image_id == 0:
                        ...
                        random.shuffle(list(tmp))//让训练数据集中的数据打乱顺序，然后一个挨着一个地(for i in indices)生成训练数据对
                        train_low_data, train_high_data  = zip(*tmp)//从tmp中依次取一个元组，组成一个元组
